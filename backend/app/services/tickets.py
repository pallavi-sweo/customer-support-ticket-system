...
from app.policies.tickets import can_reply_ticket, can_view_ticket, can_update_status
...
def create_reply_service(db: Session, ticket_id: int, current_user, message: str):
    ...
    return create_reply(db, ticket_id=ticket_id, author_id=current_user.id, message=message)
