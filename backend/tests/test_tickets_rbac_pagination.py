...
def test_user_can_create_and_list_own_tickets(client):
    ...
    r = client.get(
        "/tickets?page=1&page_size=10", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_user_cannot_access_others_ticket(client):
    ...
    r = client.get(
        f"/tickets/{created['id']}", headers={"Authorization": f"Bearer {t2}"}
    )
...
def test_pagination_works(client):
    ...
    r2 = client.get(
        "/tickets?page=2&page_size=10", headers={"Authorization": f"Bearer {token}"}
    )
